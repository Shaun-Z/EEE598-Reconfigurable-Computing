...
parameter LENGTH_a = 256;
parameter LENGTH_b = 256;
parameter LENGTH_prod = 256;
...
wire    [7 : 0]    arraya_address0, arraya_address1;
...
wire    [7 : 0]    arrayb_address0, arrayb_address1;
...
wire [4 - 1 : 0]    arrayprod_we0, arrayprod_we1;
wire    [7 : 0]    arrayprod_address0, arrayprod_address1;
wire    [31 : 0]    arrayprod_din0, arrayprod_din1;
wire    [31 : 0]    arrayprod_dout0, arrayprod_dout1;
...
assign arrayprod_we0[2] = prod_we0;
assign arrayprod_we0[3] = prod_we0;
...
reg dump_tvout_finish_prod;

initial begin : dump_tvout_runtime_sign_prod
    integer fp;
    dump_tvout_finish_prod = 0;
    fp = $fopen(`AUTOTB_TVOUT_prod_out_wrapc, "wb");
    if (fp == 0) begin
        $display("Failed to open file \"%s\"!", `AUTOTB_TVOUT_prod_out_wrapc);
    ...
    fp = $fopen(`AUTOTB_TVOUT_prod_out_wrapc, "ab");
    if (fp == 0) begin
        $display("Failed to open file \"%s\"!", `AUTOTB_TVOUT_prod_out_wrapc);
        $display("ERROR: Simulation using HLS TB failed.");
        $finish;
